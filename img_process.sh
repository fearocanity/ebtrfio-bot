...
case "${1}" in
	in_progress)
		...
		convert -fill white -background "#a26b03" -gravity center -pointsize 72 -font "trebuc.ttf" label:"\ [~] Frame ${prev_frame}-${lim_frame} was currently posting in progress " -append -bordercolor "#a26b03" -border 30 status/status.jpg
		;;
	failed)
		shift 1
		lim_frame="$((prev_frame+fph-1))"
